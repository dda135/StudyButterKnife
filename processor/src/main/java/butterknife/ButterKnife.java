 ...
 * </code></pre>
 * 稍微总结一下，可以通过注解的方式来绑定视图和对应的资源ID，并且可以绑定一些定义的事件和视图ID、
 * 也可以绑定某个参数和对应的资源ID。
 * 注意要正常使用必须让ButterKnife绑定指定的对象，可以是Activity、Dialog等
 */
public final class ButterKnife {
  ...
  public static Unbinder bind(@NonNull Activity target) {
    //获取当前Activity对应的Window的DecorView
  ...
  /**
   * 建立绑定关系
   * @param target 需要绑定的对象，一般为bind的参数对象
   * @param source 需要绑定的顶层视图
   * @return
     */
  ...
  private static Constructor<? extends Unbinder> findBindingConstructorForClass(Class<?> cls) {
    //尝试从缓存中获取对应的构造函数，这里只有解析过某一个cls的情况下才有值
    ...
    if (clsName.startsWith("android.") || clsName.startsWith("java.")) {//过滤Java和Android的内部类
      if (debug) Log.d(TAG, "MISS: Reached framework class. Abandoning search.");
      return null;
    }
    try {
      //通过ClassLoader读取名称为MainActivity_ViewBinding的类（一个例子）
      //其中这个文件是在编译期间的ButterKnifeProcessor中生成的
      Class<?> bindingClass = cls.getClassLoader().loadClass(clsName + "_ViewBinding");
      //noinspection unchecked
      //获取指定的构造函数，其中有两个Class参数(target,view)
    ...
    //放入缓存当中，再次拉起当前页面的时候，可以直接从缓存中获取构造函数，从而节省反射的开支
